...
contract Benchmark is Script {
    ...
    function run() public {
        ...
        address depositRtr = json.readAddress(
            ".transactions[1].additionalContracts[0].address"
        );

        address positionRtr = json.readAddress(
            ".transactions[2].additionalContracts[0].address"
        );
        ...
        vm.startBroadcast();
        vault.depositEther{value: .00002 ether}();
        console.logAddress(depositRtr);
        console.logAddress(positionRtr);

        (bool ok, ) = payable(depositRtr).call{value: .00002 ether}("");
        ...
        uint256 amount = .000000025 ether;
