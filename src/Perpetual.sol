...
contract PerpetualLongInput is PerpetualBaseRouter {
    ...
    function _openShortOutput(
        ...
        PERPETUAL_CLEARING_HOUSE.openPositionFor(
            ...
            IClearingHouse.OpenPositionParams({
                ...
                amount: amount,
                oppositeAmountBound: oppositeAmountBound,
                deadline: type(uint256).max, // TODO: verify this is market order behavior, and do we need a separate limit router
                sqrtPriceLimitX96: sqrtPriceLimitX96,
                referralCode: REFERRAL_CODE
            })
        );
    ...
    function _closePosition(
        ...
        int256 takerPositionSize = ACCOUNT_BALANCE.getTakerPositionSize(
            msg.sender,
            TOKEN
        );
    ...
    fallback() external payable {
        ...
        if (funcName == 1)
            _openShortOutput(amount, oppositeAmountBound, sqrtPriceLimitX96);
        else if (funcName == 2)
            _openShortInput(amount, oppositeAmountBound, sqrtPriceLimitX96);
        else if (funcName == 3)
            _openLongOutput(amount, oppositeAmountBound, sqrtPriceLimitX96);
        else if (funcName == 4)
            _openLongInput(amount, oppositeAmountBound, sqrtPriceLimitX96);
        else if (funcName == 5)
            _closePosition(oppositeAmountBound, sqrtPriceLimitX96);
