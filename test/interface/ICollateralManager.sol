...
// Copied from:
// https://optimistic.etherscan.io/address/0x840048281d438bc834fa5c8a3d0ba7822553bc88#code#F11
...
interface ICollateralManager {
  ...
  function getCollateralConfig(address token) external view returns (Config memory);
  ...
  /// @notice Get the minimum `margin ratio - mmRatio` before the account's collateral is
  /// liquidatable
  /// @dev 6 decimals, same decimals as _mmRatio
  /// @return ratio
  function getMmRatioBuffer() external view returns (uint24);

  /// @notice Get the maximum `debt / nonSettlementTokenValue` before the account's collaterals are
  /// liquidated
  /// @dev 6 decimals
  /// @return ratio
